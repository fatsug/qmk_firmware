...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	[_BASE] = LAYOUT(
        ...
        KC_LCTL,  KC_LGUI, TD(ALT_OSL1),                 KC_SPC,                             KC_LOWER, KC_RAISE, KC_LEFT, KC_DOWN, KC_RGHT
    ),
    ...
    [_ADJUST] = LAYOUT(
        ...
        XXXXXXX,          XXXXXXX,  XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, XXXXXXX, KC_QWERTY, KC_COLEMAK, KC_ASTG, XXXXXXX, XXXXXXX, \
        _______,  _______, _______,                            KC_A,                           _______, _______, _______, _______, _______
    )
};

/*
Tap Dance and One Shot Layer
*/
int cur_dance (qk_tap_dance_state_t *state) {
  if (state->count == 1) {
...
void alt_finished (qk_tap_dance_state_t *state, void *user_data) {
  ...
  switch (alttap_state.state) {
    case SINGLE_TAP: set_oneshot_layer(_LOWER, ONESHOT_START); clear_oneshot_layer_state(ONESHOT_PRESSED); break;
    case SINGLE_HOLD: register_code(KC_LALT); break;
    case DOUBLE_TAP: set_oneshot_layer(_LOWER, ONESHOT_START); set_oneshot_layer(_LOWER, ONESHOT_PRESSED); break;
    case DOUBLE_HOLD: register_code(KC_LALT); layer_on(_LOWER); break;
...
void alt_reset (qk_tap_dance_state_t *state, void *user_data) {
  switch (alttap_state.state) {
    ...
    case DOUBLE_HOLD: layer_off(_LOWER); unregister_code(KC_LALT); break;
  }
